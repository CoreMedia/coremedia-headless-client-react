...
async function record(req, res, data, options, pathParams, queryParams) {
  ...
  if (data && ext === "json") {
    try {
      ...
      logger.warn("Cannot parse JSON from response");
    }
  }
  ...
  try {
    ...
    logger.error(`Cannot save mock: ${error && error.message}`);
  }
}
...
function buildFile(mock, depth) {
  ...
  let pathComponents = reqPath.split("/").filter((c) => c);
  if (mock.pathParams) {
    pathComponents = pathComponents.concat(mock.pathParams);
  }
  ...
  logger.info("Build mock for " + file);
